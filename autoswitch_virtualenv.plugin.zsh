...
function _maybeworkon() {
    ...
    if [[ -z "$VIRTUAL_ENV" || "$venv_name" != "$(basename $VIRTUAL_ENV)" ]]; then
        ...
        if [[ ! -d "$venv_dir" ]]; then
            printf "Cannot find virtualenv ${PURPLE}%s${NORMAL}\n" "$venv_name"
            printf "Consider running ${PURPLE}mkvenv --force${NORMAL}\n"
            return
        fi
     ...
     if ! source "$(_virtual_env_dir)/$venv_name/bin/activate"; then
         printf "Failed to load virtualenv\n"
         printf "If this issue persists - run ${PURPLE}rmvenv && mkvenv${NORMAL} in this directory\n"
     fi
  fi
}
...
function _default_venv()
{
    if [[ -n "$AUTOSWITCH_DEFAULTENV" ]]; then
        ...
        printf "deactivating ${PURPLE}$(basename $VIRTUAL_ENV)${NORMAL}\n"
        deactivate
    fi
}
